...
public class Controlador {
	...
	*/
	...
	/*"/ ó /welcome" es la página que queremos mostrar al inicio, el recurso a mostrar y que tendremos que escribir
	en el navegador después de
	Localhost:9000 o nada o welcome @RequestMapping indica que el método justo debajo será el que atenderá
	la petición tipo Get (cuando solo hay una se supone tipo Get, cuando sea Post (formulario) tendremos dos métodos.
	En el próximo método, veremos que en lugar de usar RequestMapping usaremos GetMapping, pero por si os lo
	encontráis por el mundo de internet*/
	
		@GetMapping ({"/", "/welcome"})
	public String welcome (@RequestParam(name="nombre", required=false, defaultValue= "Mundo") String 
	nombre, Model model) {
	/*El parámetro "nombre" es la palabra que usamos para darle un nombre a la variable y debemos usar en la plantilla
	dentro de <p>Hello <span th:text="${nombre}">Friend</span>!</p>
	Ojo porque no tener un criterio a la hora de dar estos nombres a las variables hace que tengamos muchos errores (usar
	mayúsculas o minúsculas, que el nombre no indique qué guarda, mezclas varias palabras... */
	model.addAttribute("nombre", nombre);//Incluimos la información en el modelo
	return "index";//Nombre de la plantilla que generará la página HTML (sin extensión), en nuestro caso
	//estos html deben estar
	//dentro de templates y llamarse igual que el String que devuelve el método, index.html Ojo con las
	//máyúsculas y minúsculas del nombre
	//de la plantilla porque son tenidas en cuenta
	}
		
		
	//Segundo ejemplo, donde se pueden ver distintos parámetros que se le pasan al model, como un objeto de la
	//clase
	//Persona, con valores de sus dos atributos. Más adelante iremos viendo de dónde sacamos esa "información" 
